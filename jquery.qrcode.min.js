...
(function($){function str2rstr_utf8(input)
...
QRCode.prototype={addData:function(data){data=str2rstr_utf8(data);var newData=new QR8bitByte(data);this.dataList.push(newData);this.dataCache=null;},isDark:function(row,col){if(row<0||this.moduleCount<=row||col<0||this.moduleCount<=col){throw new Error(row+","+col);}
...
this.length++;}};$.fn.qrcode=function(options){if(typeof options==='string'){options={text:options};}
